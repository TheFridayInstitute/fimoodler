...
### Questionnaires
Questionnaires are added to courses as course modules.  You can figure out which questionnaires are in a course with `list_cms`.  You can also view the questions and response choices for a specific questionnaire with `view_questionnaire`.

You can retrieve responses to questionnaire questions with the `fetch_questionnaires` function.  You can control whether or not to include responses from partially complete questionnaires.  The most recent responses are returned.  Note that a questionnaire is considered complete when the entire questionnaire is submitted, even if a non-required question is left blank.

```{r eval=FALSE}
# Working memory unit feedback survey, including incomplete responses
responses <- fetch_questionnaires(
  qnr_cm_id = 4883,
  user_ids = c(16122, 15302, 15153, 15796, 15831, 15698)
...
### Quizzes
Quizzes are added to courses as course modules.  You can figure out which quizzes are in a course with `list_cms`. Note that micro-credentials quizzes are located in a separate micro-credential "course" (courseid 15).  MOOCs that include micro-credentials actually link to quizzes in the micro-credential course.

Final quiz grades can be retrieved with the `fetch_quiz_grades` function.  Note, this returns the most recent grade for each user.  You can only see past grades if you dig into the logs.  Note that the *final* grades reported here are scaled, if scaling was specified in the quiz.  After summing the individual quiz question grades, Moodle is capable of scaling the sum.  For example, a quiz may be scored on a 5 point scale (grade sums ranging 0-5), but scaled to a range of 0-20 for contribution to final grade in a class.  If 100 points can be earned according to they syllabus, then a range of 0-20 would mean the quiz is 20% of the final course grade.

```{r eval=FALSE}
# Working memory micro-credential grades
user_ids <- c(16122, 15302, 15153, 15796, 15831, 15698)
grades <- dplyr::tibble(userid = user_ids) %>%
  dplyr::bind_cols(
    fetch_quiz_grades(quiz_cm_id = 3060, user_ids = user_ids))
```
